...
// V66 자막 싱크 전면 재정리: API 없이 브라우저 파형만 사용한다.
// 핵심: 말소리 조각 경계에 자막을 억지로 붙이지 않고, 전체 음성 길이 + 자막 읽기 가중치로 순차 배치한다.
const CAPTION_FIRST_START_DELAY = 0.00;
const CAPTION_SINGLE_START_DELAY = 0.00;
const VOICE_CUT_DELAY = 0.00;

// 짧은 단어 쉼 때문에 싱크가 깨지지 않도록 파형 조각은 시작점 참고용으로만 사용한다.
const VOICE_SEGMENT_MERGE_GAP = 0.35;
const VOICE_BOUNDARY_MIN_GAP = 0.80;
const VOICE_MIN_SEGMENT_LENGTH = 0.12;
...
function getCaptionSyncProfile(text) {
  const raw = String(text || '');
  const key = plainKey(raw);
  const len = key.length;
  const hasQuestion = /[?？]|(나요|까요|뭘까|왜|어떻게|혹시)/.test(raw);
  const hasComma = /[,，、]|(그런데|하지만|그리고|그래서|먼저|그다음)/.test(raw);
  const hasEmotion = /(마음|위로|힘들|지친|외로|무너|버텨|괜찮|상처|눈물|소중|혼자|밤)/.test(raw);
  const hasInfo = /(기준|방법|체크|확인|정리|이유|상식|비용|손해|문제|위험|필요|먼저)/.test(raw);
  const hasCTA = /(저장|공유|댓글|확인하세요|기억하세요|보세요|눌러|구독)/.test(raw);

  if (len <= 8) return { type: 'veryShortHook', speedMul: 1.28, pause: 0.16, minExtra: 0.18 };
  if (len <= 14) return { type: 'shortHook', speedMul: 1.16, pause: 0.12, minExtra: 0.12 };
  if (hasQuestion) return { type: 'question', speedMul: 1.04, pause: 0.28, minExtra: 0.10 };
  if (hasEmotion) return { type: 'emotion', speedMul: 0.94, pause: 0.34, minExtra: 0.18 };
  if (hasCTA) return { type: 'cta', speedMul: 0.98, pause: 0.30, minExtra: 0.12 };
  if (hasInfo) return { type: 'info', speedMul: 1.08, pause: 0.18, minExtra: 0.04 };
  if (hasComma) return { type: 'pauseSentence', speedMul: 1.00, pause: 0.24, minExtra: 0.08 };
  if (len >= 28) return { type: 'longSentence', speedMul: 1.10, pause: 0.12, minExtra: 0.00 };
  return { type: 'normal', speedMul: 1.00, pause: 0.18, minExtra: 0.06 };
}

function estimateCaptionReadWeight(text, minDur = 1.5) {
  const raw = String(text || '');
  const profile = getCaptionSyncProfile(raw);
  ...
  const commaPause = (raw.match(/[,，、]/g) || []).length * 0.24;
  const sentencePause = (raw.match(/[.!?。！？…]/g) || []).length * 0.30;
  const linePause = (raw.match(/\n/g) || []).length * 0.18;
  const spacePause = Math.min(0.42, (raw.match(/\s/g) || []).length * 0.03);

  // V65 상황별 싱크: 감성문장은 조금 길게, 정보형은 조금 빠르게, 질문/후킹/마무리는 말끝 여백을 준다.
  const syllableWeight = (koreanLen * 0.116 + englishWords * 0.30 + numberLen * 0.055) / profile.speedMul;
  const lengthCurve = keyLen >= 32 ? 0.18 : keyLen >= 24 ? 0.10 : 0;
  const weight = syllableWeight + commaPause + sentencePause + linePause + spacePause + profile.pause + profile.minExtra + lengthCurve + 0.42;
  return Math.max(minDur + profile.minExtra, weight);
}
...
function distributeCaptionDurations(parts, totalDuration, minDur = 1.5) {
  ...
  const safeMin = Math.min(Math.max(0.48, avg * 0.36), minDur);
  const safeMax = Math.max(safeMin + 0.35, avg * 2.35);
...
function getSpeechSegments(profile, totalDuration) {
  ...
  const segments = (profile?.segments || [])
    ...
    .filter((seg) => seg.end - seg.start >= VOICE_MIN_SEGMENT_LENGTH)
  ...
  segments.forEach((seg) => {
    ...
    if (last && seg.start - last.end <= VOICE_SEGMENT_MERGE_GAP) last.end = Math.max(last.end, seg.end);
...
function findSilenceGapAt(time, segments, totalDuration) {
  ...
  if (t < segments[0].start && segments[0].start >= VOICE_BOUNDARY_MIN_GAP) {
  ...
  for (let i = 0; i < segments.length - 1; i++) {
    ...
    if (gapEnd - gapStart < VOICE_BOUNDARY_MIN_GAP) continue;
  ...
  if (t >= last.end && total - last.end >= VOICE_BOUNDARY_MIN_GAP) {
...
async function analyzeAudioSilenceProfile(file) {
  ...
  try {
    ...
    raw.forEach((seg) => {
      ...
      const last = merged[merged.length - 1];
      if (last && seg.start - last.end < VOICE_SEGMENT_MERGE_GAP) last.end = seg.end;
      else merged.push({ ...seg });
    });
...
function findVoiceCutCandidates(segments, totalDuration) {
  const total = Math.max(0.5, Number(totalDuration) || 0);
  // 음성과 동시에 맞추기: 자막 전환 후보는 무조건 다음 말소리 시작점만 사용한다.
  // 이전 말끝을 후보로 쓰면 무음구간에서 다음 자막이 먼저 떠서 싱크가 틀어질 수 있다.
  return (segments || [])
    .slice(1)
    .map((seg) => clamp(Number(seg.start || 0) + VOICE_CUT_DELAY, 0, total))
    .filter((v, i, arr) => Number.isFinite(v) && v > 0.01 && v < total - 0.01 && (!i || Math.abs(v - arr[i - 1]) >= VOICE_BOUNDARY_MIN_GAP));
}

function pickVoiceBoundaryIndexes(parts, segments, totalDuration, baseCaptions) {
  const total = Math.max(0.5, Number(totalDuration) || 0);
  const cleanParts = parts || [];
  const count = cleanParts.length;
  if (!segments.length || count <= 1) return [];
  if (segments.length < count) return [];

  // 말소리 구간 수가 자막 수와 비슷하면, 각 자막 시작을 말소리 시작점에 직접 붙인다.
  if (segments.length >= count && segments.length <= count + 2) {
    return Array.from({ length: count }, (_, i) => i);
  }

  const weights = cleanParts.map((text) => estimateCaptionReadWeight(text, 0.4));
  ...
  for (let i = 1; i < count; i++) {
    acc += weights[i - 1];
    const targetRatio = acc / sum;
    const baseTarget = baseCaptions?.[i]?.start || (total * targetRatio);
    const ratioTarget = total * targetRatio;
    const target = (baseTarget * 0.45) + (ratioTarget * 0.55);
    const minIdx = indexes[indexes.length - 1] + 1;
    const maxIdx = segments.length - (count - i);
    let bestIdx = minIdx;
    let bestDist = Infinity;
    for (let j = minIdx; j <= maxIdx; j++) {
      const dist = Math.abs(Number(segments[j]?.start || 0) - target);
      if (dist < bestDist) {
        bestDist = dist;
        bestIdx = j;
      }
...
function getNextSpeechStart(time, segments, totalDuration) {
  const total = Math.max(0.5, Number(totalDuration) || 0);
  const t = clamp(Number(time) || 0, 0, total);
...
function alignCaptionsToVoiceStarts(captions, segments, totalDuration) {
  const total = Math.max(0.5, Number(totalDuration) || 0);
  if (!captions?.length || !segments?.length) return captions;

  const aligned = captions.map((cap) => ({ ...cap, captionStyle: { ...(cap.captionStyle || {}) } }));
  const count = aligned.length;

  // 핵심: 자막 문구는 그대로 두고, 자막 시작만 말소리 시작점 이후로 보낸다.
  // 말소리 나오기 전 빈 구간에서는 다음 자막이 먼저 뜨지 않는다.
  if (segments.length >= count && segments.length <= count + 2) {
    for (let i = 0; i < count; i++) {
      const seg = segments[i] || segments[segments.length - 1];
      aligned[i].start = Number(clamp(Number(seg.start || 0), 0, total).toFixed(2));
      aligned[i].end = Number((i < count - 1
        ? clamp(Number(segments[i + 1]?.start || aligned[i].end), aligned[i].start + 0.18, total)
        : total).toFixed(2));
    }
    return aligned;
  }

  aligned[0].start = Number(clamp(Number(segments[0]?.start || 0), 0, total).toFixed(2));
  for (let i = 1; i < count; i++) {
    const currentStart = Number(aligned[i].start || 0);
    const voiceStart = getNextSpeechStart(currentStart, segments, total);
    const shiftedStart = Math.max(currentStart, voiceStart);
    aligned[i].start = Number(clamp(shiftedStart, aligned[i - 1].start + 0.18, total).toFixed(2));
    aligned[i - 1].end = aligned[i].start;
  }
  aligned[count - 1].end = Number(total.toFixed(2));

  for (let i = 0; i < count; i++) {
    if (aligned[i].end <= aligned[i].start) aligned[i].end = Math.min(total, aligned[i].start + 0.18);
    aligned[i].start = Number(aligned[i].start.toFixed(2));
    aligned[i].end = Number(aligned[i].end.toFixed(2));
  }
  return aligned;
}
...
function buildVoiceSegmentCaptionTimeline(parts, totalDuration, style, audioProfile) {
  ...
  // V67 싱크 리셋: 자막 텍스트는 그대로 유지하고, 표시 시작만 실제 말소리 시작점에 붙인다.
  // 중간 자막이 목소리보다 먼저 뜨는 문제를 막기 위해, 자막 시작 시간이 무음구간이면 다음 말소리 시작점까지 뒤로 민다.
  const segments = getSpeechSegments(audioProfile, total);
  const firstVoiceAt = Number(segments?.[0]?.start || audioProfile?.firstVoiceAt || 0);
  const startAt = Number(clamp(firstVoiceAt, 0, Math.min(total - 0.3, 1.2)).toFixed(2));
  const activeTotal = Math.max(0.5, total - startAt);
  const minDur = clamp(parseFloat($('captionMinSeconds')?.value || '1.5'), 1.0, 4);
  const durations = distributeCaptionDurations(cleanParts, activeTotal, minDur);
  ...
  cleanParts.forEach((text, idx) => {
    const isLast = idx === cleanParts.length - 1;
    const start = Number(t.toFixed(2));
    const rawEnd = isLast ? total : Math.min(total, t + durations[idx]);
    const end = Number(Math.max(start + 0.18, rawEnd).toFixed(2));
    captions.push({
      id: uid(),
      start,
      end,
  ...
  if (captions.length) captions[captions.length - 1].end = Number(total.toFixed(2));
  return alignCaptionsToVoiceStarts(captions, segments, total);
}
...
async function analyzeAudioAndCaptions() {
  ...
  setAudioAnalysisProgress(100, '분석 완료. 장면 시간이 음성 길이 기준으로 생성됐습니다.');
  markAudioAnalysisReady(true);
  const silenceCount = Math.max(0, segments.length - 1);
  log(`음성·자막 분석 완료: 음성 ${formatTime(duration)}, 자막 ${parts.length}개, 말소리 구간 ${segments.length || '기본'}개, 무음 후보 ${silenceCount}개. 장면은 자막 읽기 길이 기준으로 순차 생성했습니다.`);
  return true;
}
...
function createCaptionsFromScript() {
  ...
  state.captions = state.audioProfile
    ? buildVoiceSegmentCaptionTimeline(parts, total, style, state.audioProfile)
    : buildCaptionTimeline(parts, total, style, state.audioProfile);
  state.audioDuration = total;
  rebuildScenes();
  if ($('generatedScript')) $('generatedScript').value = parts.join('\n\n');
  if ($('ttsText')) $('ttsText').value = parts.join('\n');
  ...
  log(`${state.audioProfile ? '음성 길이 기준 순차 싱크' : '대본 기준 싱크'} 적용 완료: 음성 ${formatTime(total)}, 자막 ${parts.length}개`);
}
