...
const CAPTION_FIRST_START_DELAY = 0.12;
const CAPTION_SINGLE_START_DELAY = 0.10;
const VOICE_CUT_DELAY = 0.03;
...
const state = {
  ...
  captionGlobalOffset: 0,
  firstCaptionOffset: 0,
  playing: false,
  playStartMs: 0,
  playStartTime: 0,
  raf: null,
...
function applyVoiceAnalysisCaptions(parts, totalDuration, style = collectCaptionStyle()) {
  const total = Math.max(0.5, Number(totalDuration) || 0);
  // V66: 실제 음성 분석 결과를 자막 시간표에 적용한다.
  // 기존에는 파형 분석을 해도 buildStableCaptionTimeline()만 써서 자막이 음성보다 먼저 뜰 수 있었다.
  // 이제 말소리 시작점/무음 후보를 반영하고, 실패 시에만 기존 안정형 시간표로 fallback한다.
  const captions = buildVoiceSegmentCaptionTimeline(parts, total, style, state.audioProfile)
    || buildStableCaptionTimeline(parts, total, style);
  state.captionGlobalOffset = 0;
  state.firstCaptionOffset = 0;
...
async function renderWebm() {
  ...
  try {
    ...
    function finishRender() {
      ...
      if (!senialConsumeDailyUse('영상 다운로드')) {
        updateRenderProgress(100, `완료 100% - 오늘 다운로드 한도 초과`);
        if (status) status.textContent = `완료됐지만 오늘 다운로드 한도를 초과했습니다.`;
        if (link) {
          link.classList.add('disabledDownload');
          link.removeAttribute('href');
          link.textContent = '오늘 다운로드 한도 초과';
        }
        return;
      }
...
const SENIAL_SEO_PROMPT_GUIDES = {
  ...
  '부업': '반드시 실제 실행 방법을 설명한다. 부업 이름만 소개하지 말고 어디서 가입하는지, 어떻게 시작하는지, 수익이 발생하는 구조는 무엇인지, 초보자가 주의할 점은 무엇인지 포함한다. 예를 들어 위탁판매라면 공급처 가입, 상품 등록, 주문 처리, 공급처 배송, 판매가와 공급가 차액 구조를 설명한다. 시청자가 영상을 보고 바로 따라할 수 있을 정도로 구체적으로 작성한다.',
...
[핵심 내용 필수 규칙]
- 선택된 제목이 무엇을 말하는지 3문장 안에 분명히 드러내기
- 정보형 주제는 시청자가 영상을 다 보고도 '그래서 어떻게 하는데'라는 의문이 남지 않게 작성
- 정보형 주제는 반드시 무엇인지, 실제 하는 방법, 작동 원리 또는 수익 구조, 초보자 주의사항, 시작 순서 중 최소 3개 이상 포함
- 부업, 돈관리, 사업, 창업, 자기계발, 건강상식, 상식, 역사, 심리학, 여행, 뉴스는 감성 문장만 나열 금지
- 부업 주제는 실제 부업 이름, 어디서 시작하는지, 무엇을 등록하거나 실행하는지, 수익이 어떻게 생기는지, 초보자가 피할 점을 반드시 포함
- 위탁판매 주제는 공급처 가입, 상품 선택, 스마트스토어 또는 쇼핑몰 등록, 주문 발생, 공급처 배송, 판매가와 공급가 차액 구조를 반드시 설명
- 블로그 주제는 어떤 글을 쓰는지, 어디서 수익이 생기는지, 초보자가 무엇부터 해야 하는지 설명
- 스마트스토어 주제는 상품 소싱, 상품 등록, 주문 처리, 배송 방식, 마진 구조 중 최소 3개 포함
- 돈관리 주제는 고정비, 소비 습관, 현금흐름, 실행 기준 중 최소 2개 포함
- 사업/창업 주제는 고객, 매출, 검증, 온라인 세팅, 전환 중 최소 2개 포함
- 건강상식/다이어트 주제는 생활 습관 중심으로 원인과 실천법을 포함하되 의료 단정 금지
- 역사/상식/심리학 주제는 사건, 이유, 오늘 적용점이 드러나게 작성
- 여행 주제는 장소, 준비 팁, 실수 방지, 감정 중 최소 2개 포함
- 핵심 정보 없이 '좋습니다', '해보세요', '중요합니다', '작게 시작하세요', '꾸준히 하면 됩니다'만 반복 금지
- 정보형 주제는 실제 방법 설명 비중을 70퍼센트 이상으로 작성
- 공유해주세요, 댓글 남겨주세요, 저장해두세요 같은 CTA는 마지막 1문장에만 사용
...
1문장 강한 후킹
2문장 안에 핵심 방법 공개
3~4문장 무엇인지와 작동 원리 설명
중간 문장 실제 하는 방법, 순서, 수익 구조, 예시, 주의점
마지막 전 문장 초보자 실행 요약
마지막 문장 저장/댓글 유도
공감 문장은 최대 2문장까지만 사용하고, 핵심 설명을 뒤로 미루지 않기
...
[좋은 예시 방향]
위탁판매는 재고를 먼저 사두지 않는 판매 방식입니다
도매매나 오너클랜 같은 공급처에서 상품을 고릅니다
그 상품을 스마트스토어나 쇼핑몰에 등록합니다
주문이 들어오면 공급처에 주문을 넣습니다
공급처가 고객에게 직접 배송합니다
판매가와 공급가 차액이 내 수익입니다
초보자는 생활용품처럼 반복 구매가 있는 상품부터 테스트합니다

지금 바로 핵심 정보가 들어간 대본만 작성해라.`;
}
...
/* SENIAL_ACCESS_GATE_V1: 무료 배포 코드 제한. 기존 대본/싱크/이미지 로직과 분리 */
const SENIAL_ACCESS_SALT = 'SENIAL_V65_FREE_BETA_2026';
const SENIAL_ACCESS_DAYS = 30;
const SENIAL_DAILY_LIMIT = 30;
const SENIAL_ACCESS_CODES = {"2875338100330973":"SN001","1220172596700031":"SN002","8437932531255888":"SN003","4961886751475083":"SN004","2848996849284785":"SN005","2809503996213583":"SN006","4092545176135645":"SN007","6706909695104722":"SN008","2531654523767525":"SN009","8617440509246112":"SN010","8487077366906912":"SN011","1488619303238159":"SN012","2892122008929695":"SN013","2765416519881241":"SN014","8746575815317367":"SN015","3672929174280201":"SN016","1624080408371103":"SN017","8016889126934541":"SN018","8826145481277549":"SN019","5494887812013601":"SN020","7308989866731604":"SN021","4712520300444047":"SN022","4745373759399848":"SN023","212511227736436":"SN024","7446542148370325":"SN025","3632410753713699":"SN026","1362652346717178":"SN027","2886938295494726":"SN028","3354185796027967":"SN029","7614673530851754":"SN030","4138093936827112":"SN031","7376529630053753":"SN032","2724162582159737":"SN033","7160721423473620":"SN034","213456788689491":"SN035","1546726251359466":"SN036","3236495698215129":"SN037","493796232927418":"SN038","6882007339590936":"SN039","6475874569992202":"SN040","3721585597760019":"SN041","3879590648679807":"SN042","1229796236145516":"SN043","4173432594742215":"SN044","5549737816992459":"SN045","7610299078964277":"SN046","2970623521657427":"SN047","6034030237896616":"SN048","2250692579178138":"SN049","4418986765673723":"SN050","2076122460644796":"SN051","6254432905008533":"SN052","6568254762139060":"SN053","2830988856861319":"SN054","3952759514162796":"SN055","2850692611872674":"SN056","5549593709270773":"SN057","8128521607556551":"SN058","2377856295730105":"SN059","3626095768027021":"SN060","8390406028207910":"SN061","2234613218094634":"SN062","3876983034575994":"SN063","399661058610064":"SN064","724243552552928":"SN065","4747774309486802":"SN066","8035631303958606":"SN067","6586384213473368":"SN068","7550678642788144":"SN069","7960633628872082":"SN070","2134327772903278":"SN071","6440843005404617":"SN072","7526930407433803":"SN073","5178782935742317":"SN074","6080807110843420":"SN075","3937424242581197":"SN076","6604402277865165":"SN077","8730041032999374":"SN078","7738311965057502":"SN079","2252013525846612":"SN080","6494558917133027":"SN081","4987155048292279":"SN082","5372428676076752":"SN083","5685152219194988":"SN084","5350021002272045":"SN085","7103723060426966":"SN086","4207710429343614":"SN087","1204553702566274":"SN088","8497381331964495":"SN089","6342619200349733":"SN090","591618953220324":"SN091","5928551981153921":"SN092","1253665536804230":"SN093","5817492895782677":"SN094","2598531098667062":"SN095","8534870499934065":"SN096","3609613928383533":"SN097","5799960179000411":"SN098","1876705476712432":"SN099","2880218566991854":"SN100","843463070114654":"SN101","1609847533156013":"SN102","5770573691089576":"SN103","4362097175212343":"SN104","6052262785876208":"SN105","8331049802540477":"SN106","6382463871509049":"SN107","3638826027456033":"SN108","984255869461197":"SN109","6405892721031613":"SN110","3538004037827133":"SN111","797216617314924":"SN112","3993337911365811":"SN113","4203839681584062":"SN114","2289089599468535":"SN115","7716551021785945":"SN116","3774876016186384":"SN117","5444241151516400":"SN118","5087586379707567":"SN119","8305289899190904":"SN120","288403069482257":"SN121","472166943485997":"SN122","1334851104365049":"SN123","2336752591567118":"SN124","890648503515240":"SN125","1242084787416411":"SN126","965722939010366":"SN127","4721694886446453":"SN128","4239798496386978":"SN129","1519289253812309":"SN130","7194553343826911":"SN131","2482454865299364":"SN132","8073139008747924":"SN133","6758654919804155":"SN134","2172913035508604":"SN135","2429920261028867":"SN136","6865563160010200":"SN137","1078085325027906":"SN138","945263586836511":"SN139","6790256748410829":"SN140","4451962486255125":"SN141","8728698346249321":"SN142","1881041009873142":"SN143","5369531743449156":"SN144","4768967211118891":"SN145","3094102396968390":"SN146","936048864330973":"SN147","4838076549390048":"SN148","6628606119667181":"SN149","1755923830338963":"SN150","8861427195806628":"SN151","5816426150115037":"SN152","4559309402385427":"SN153","3829378236662431":"SN154","231087151572556":"SN155","6670422367451662":"SN156","259326701786541":"SN157","4170291645174202":"SN158","8256605115861879":"SN159","4347156484384877":"SN160","2581783325539160":"SN161","2665513566916542":"SN162","4871246682077215":"SN163","5676910916527245":"SN164","8311145661301216":"SN165","4721217067095577":"SN166","2274373348294253":"SN167","7016181138181798":"SN168","211507671375975":"SN169","7943807180726146":"SN170","4816850333762":"SN171","6160616659473339":"SN172","1439406746678280":"SN173","7160786414260502":"SN174","3925569245254152":"SN175","929309023549273":"SN176","4415523502328509":"SN177","4661450318399296":"SN178","7487817355169224":"SN179","3538917196703614":"SN180","288786007120109":"SN181","4712928018528698":"SN182","7958199432781370":"SN183","7859699208773376":"SN184","524751158250833":"SN185","8299553554638681":"SN186","5515004106427595":"SN187","4326364736863666":"SN188","5438011184520857":"SN189","4535273172220622":"SN190","6933436908253294":"SN191","4857876376443156":"SN192","5914636631284885":"SN193","854199150338993":"SN194","510987210052791":"SN195","3158604807093656":"SN196","1452403823435112":"SN197","2780104097103417":"SN198","2764038008631456":"SN199","2395366860527264":"SN200"};

function senialHashCode(str, seed = 0) {
...
function normalizeSenialAccessCode(code) {
  return String(code || '').toUpperCase().replace(/[^A-Z0-9]/g, '').replace(/^SN/, 'SN');
}

function formatSenialDate(ms) {
  const d = new Date(Number(ms) || Date.now());
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

function getSenialDeviceId() {
  let id = localStorage.getItem('senial_device_id');
  if (!id) {
    id = `DEV-${Math.random().toString(36).slice(2)}-${Date.now().toString(36)}`;
    localStorage.setItem('senial_device_id', id);
  }
  return id;
}

function getSenialAccess() {
  try { return JSON.parse(localStorage.getItem('senial_access_v1') || 'null'); }
  catch (_) { return null; }
}

function setSenialAccess(data) {
  localStorage.setItem('senial_access_v1', JSON.stringify(data));
}

function getSenialTodayKey() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

function isSenialAccessValid() {
  const data = getSenialAccess();
  if (!data || !data.hash || !SENIAL_ACCESS_CODES[data.hash]) return false;
  if (data.deviceId && data.deviceId !== getSenialDeviceId()) return false;
  if (Number(data.expiresAt || 0) < Date.now()) return false;
  return true;
}

function getSenialUsage() {
  const access = getSenialAccess() || {};
  const today = getSenialTodayKey();
  if (!access.usage || access.usage.date !== today) access.usage = { date: today, count: 0 };
  return access;
}

function setSenialUsage(access) {
  setSenialAccess(access);
  updateSenialAccessBadge();
}

function senialRequireAccess(actionName = '작업') {
  if (!isSenialAccessValid()) {
    showSenialAccessGate('사용 코드가 필요합니다.');
    return false;
  }
  return true;
}

function senialConsumeDailyUse(actionName = '다운로드') {
  if (!senialRequireAccess(actionName)) return false;
  const access = getSenialUsage();
  const count = Number(access.usage?.count || 0);
  if (count >= SENIAL_DAILY_LIMIT) {
    showSenialAccessGate(`오늘 다운로드 한도 ${SENIAL_DAILY_LIMIT}회를 모두 사용했습니다. 내일 다시 사용하세요.`);
    return false;
  }
  access.usage.count = count + 1;
  access.lastAction = actionName;
  access.lastUsedAt = Date.now();
  setSenialUsage(access);
  log(`사용 코드 확인됨: 오늘 다운로드 ${access.usage.count}/${SENIAL_DAILY_LIMIT}회`);
  return true;
}

function updateSenialAccessBadge() {
  const badge = document.getElementById('senialAccessBadge');
  if (!badge) return;
  const access = getSenialUsage();
  if (!isSenialAccessValid()) {
    badge.textContent = '코드 미인증';
    badge.className = 'senialAccessBadge locked';
    return;
  }
  badge.textContent = `코드 ${access.codeId || '확인'} · 오늘 다운로드 ${access.usage?.count || 0}/${SENIAL_DAILY_LIMIT} · 만료 ${formatSenialDate(access.expiresAt)}`;
  badge.className = 'senialAccessBadge ok';
}

function showSenialAccessGate(message = '') {
  let gate = document.getElementById('senialAccessGate');
  if (!gate) {
    gate = document.createElement('div');
    gate.id = 'senialAccessGate';
    gate.innerHTML = `
      <div class="senialAccessCard">
        <h2>세니얼 사용 코드 입력</h2>
        <p>발급받은 고유코드를 입력해야 사용할 수 있습니다.</p>
        <input id="senialAccessCodeInput" placeholder="예: SN-XXXX-XXXX-XXXX" autocomplete="off" />
        <button id="senialAccessApplyBtn" type="button">코드 확인</button>
        <div id="senialAccessMsg"></div>
        <small>코드 1개 기준: 30일 사용 · 하루 다운로드 ${SENIAL_DAILY_LIMIT}회 · 이 브라우저에 고정</small>
      </div>`;
    document.body.appendChild(gate);
    gate.querySelector('#senialAccessApplyBtn')?.addEventListener('click', applySenialAccessCode);
    gate.querySelector('#senialAccessCodeInput')?.addEventListener('keydown', (e) => {
      if (e.key === 'Enter') applySenialAccessCode();
    });
  }
  const msg = gate.querySelector('#senialAccessMsg');
  if (msg) msg.textContent = message || '';
  gate.style.display = 'flex';
  setTimeout(() => gate.querySelector('#senialAccessCodeInput')?.focus(), 50);
}

function hideSenialAccessGate() {
  const gate = document.getElementById('senialAccessGate');
  if (gate) gate.style.display = 'none';
}

function applySenialAccessCode() {
  const input = document.getElementById('senialAccessCodeInput');
  const msg = document.getElementById('senialAccessMsg');
  const raw = String(input?.value || '').trim().toUpperCase();
  const compact = raw.replace(/[^A-Z0-9]/g, '');
  const pretty = compact.startsWith('SN') ? compact : `SN${compact}`;
  const chunks = pretty.replace(/^SN/, '').match(/.{1,4}/g) || [];
  const normalized = `SN-${chunks.join('-')}`;
  const hash = senialHashCode(`${SENIAL_ACCESS_SALT}|${normalized}`);
  const codeId = SENIAL_ACCESS_CODES[hash];
  if (!codeId) {
    if (msg) msg.textContent = '사용할 수 없는 코드입니다.';
    return false;
  }

  const existing = getSenialAccess();
  if (existing?.hash && existing.hash !== hash && Number(existing.expiresAt || 0) > Date.now()) {
    if (msg) msg.textContent = '이 브라우저는 이미 다른 코드에 연결되어 있습니다. 만료 후 새 코드를 사용하세요.';
    return false;
  }

  const now = Date.now();
  const data = existing?.hash === hash ? existing : {
    hash,
    codeId,
    activatedAt: now,
    expiresAt: now + SENIAL_ACCESS_DAYS * 24 * 60 * 60 * 1000,
    deviceId: getSenialDeviceId(),
    usage: { date: getSenialTodayKey(), count: 0 }
  };
  data.hash = hash;
  data.codeId = codeId;
  data.deviceId = getSenialDeviceId();
  if (!data.expiresAt) data.expiresAt = now + SENIAL_ACCESS_DAYS * 24 * 60 * 60 * 1000;
  setSenialAccess(data);
  updateSenialAccessBadge();
  hideSenialAccessGate();
  log(`사용 코드 인증 완료: ${codeId}, 만료일 ${formatSenialDate(data.expiresAt)}`);
  return true;
}

function initSenialAccessGate() {
  const style = document.createElement('style');
  style.textContent = `
    #senialAccessGate{position:fixed;inset:0;background:rgba(0,0,0,.78);z-index:99999;display:none;align-items:center;justify-content:center;padding:18px;backdrop-filter:blur(6px)}
    .senialAccessCard{width:min(420px,100%);background:#101722;border:1px solid #67e8a5;border-radius:18px;padding:22px;box-shadow:0 24px 70px rgba(0,0,0,.55);color:#f4f6fb}
    .senialAccessCard h2{margin:0 0 8px;font-size:22px}
    .senialAccessCard p{margin:0 0 14px;color:#9aa4b2;line-height:1.45}
    .senialAccessCard input{width:100%;height:48px;border-radius:12px;border:1px solid #334155;background:#07130d;color:#fff;padding:0 14px;font-size:16px;font-weight:900;letter-spacing:.5px}
    .senialAccessCard button{width:100%;height:46px;margin-top:10px;border:0;border-radius:12px;background:linear-gradient(135deg,#22c55e,#16a34a);color:#07120b;font-weight:1000;cursor:pointer}
    #senialAccessMsg{min-height:22px;margin-top:10px;color:#ffd166;font-weight:900;font-size:13px}
    .senialAccessCard small{display:block;color:#9aa4b2;line-height:1.45;margin-top:6px}
    .senialAccessBadge{display:inline-flex;align-items:center;margin-left:8px;padding:7px 10px;border-radius:999px;font-size:12px;font-weight:1000;border:1px solid #334155;white-space:nowrap}
    .senialAccessBadge.ok{background:#07130d;color:#67e8a5;border-color:#22c55e}
    .senialAccessBadge.locked{background:#1f0b0b;color:#ff9b9b;border-color:#7f1d1d}
  `;
  document.head.appendChild(style);

  const topActions = document.querySelector('.topActions') || document.querySelector('.topbar');
  if (topActions && !document.getElementById('senialAccessBadge')) {
    const badge = document.createElement('span');
    badge.id = 'senialAccessBadge';
    badge.className = 'senialAccessBadge locked';
    badge.textContent = '코드 미인증';
  ...
  updateSenialAccessBadge();
  if (!isSenialAccessValid()) showSenialAccessGate('사용 코드를 입력하세요.');
}
/* /SENIAL_ACCESS_GATE_V1 */
...
function bindEvents() {
  ...
  $('btnAutoTopicGenerate')?.addEventListener('click', () => { if (!senialRequireAccess('주제 생성')) return; pushUndoState(); generateAutoModeTopics(); });
  ...
  $('btnRender')?.addEventListener('click', () => { if (!senialRequireAccess('영상 다운로드')) return; renderWebm(); });
  $('btnAiAll')?.addEventListener('click', () => { if (!senialRequireAccess('이미지 프롬프트 생성')) return; pushUndoState(); generateAllImages(); });
  ...
  $('btnTranscribe')?.addEventListener('click', async () => {
    if (!senialRequireAccess('음성·자막 분석')) return;
    await analyzeAudioAndCaptions();
  });
  ...
  function ensureCaptionSyncOffsetPanel() {
    if ($('captionSyncOffsetPanel')) return;
    const panel = document.createElement('div');
    panel.id = 'captionSyncOffsetPanel';
    panel.style.cssText = 'margin-top:10px;background:#101722;border:1px solid var(--line,#303849);border-radius:12px;padding:10px;display:grid;grid-template-columns:1fr 1fr 1fr;gap:6px;align-items:center;font-size:12px;';
    panel.innerHTML = `
      <div style="grid-column:1/4;font-weight:900;color:var(--accent,#67e8a5);">자막 싱크 수동 보정</div>
      <button id="btnSyncAllMinus" type="button">전체 -0.1초</button>
      <button id="btnSyncAllPlus" type="button">전체 +0.1초</button>
      <input id="syncOffset" type="number" step="0.1" value="0.0" title="전체 자막 이동값" style="text-align:center;padding:8px;" />
      <button id="btnFirstSyncMinus" type="button">첫자막 -0.1초</button>
      <button id="btnFirstSyncPlus" type="button">첫자막 +0.1초</button>
      <input id="firstSyncOffset" type="number" step="0.1" value="0.0" title="첫 자막 시작 이동값" style="text-align:center;padding:8px;" />
      <button id="btnSyncReset" type="button" style="grid-column:1/4;background:#334155;">싱크 보정 초기화</button>
    `;
    const seekPanel = $('previewSeek')?.closest?.('.seekPanel');
    const previewWrap = document.querySelector('.previewWrap');
    const secPreview = $('secPreview');
    if (seekPanel) seekPanel.insertAdjacentElement('afterend', panel);
    else if (previewWrap?.parentElement) previewWrap.insertAdjacentElement('afterend', panel);
    else if (secPreview) secPreview.appendChild(panel);
  }

  function refreshCaptionSyncInputs() {
    if ($('syncOffset')) $('syncOffset').value = (Number(state.captionGlobalOffset || 0)).toFixed(1);
    if ($('firstSyncOffset')) $('firstSyncOffset').value = (Number(state.firstCaptionOffset || 0)).toFixed(1);
  }

  function refreshAfterCaptionSyncMove(focusCap = null) {
    syncScenesFromCaptions();
    renderEditor();
    refreshCaptionSyncInputs();
    const cap = focusCap || getSelectedCaption() || state.captions[0] || null;
    if (cap) {
      setActiveCaption(cap.id, false);
      syncPreviewToCaption(cap, false);
    } else {
      updateSeekUI(timelineTime());
      drawFrame(timelineTime());
  ...
  function shiftAllCaptions(delta) {
    if (!state.captions.length) return;
    pushUndoState();
    let move = Number(delta) || 0;
    const minStart = Math.min(...state.captions.map(c => Number(c.start) || 0));
    if (minStart + move < 0) move = -minStart;
    if (Math.abs(move) < 0.001) return;
    state.captions.forEach((cap) => {
      cap.start = Number(Math.max(0, (Number(cap.start) || 0) + move).toFixed(2));
      cap.end = Number(Math.max(cap.start + 0.18, (Number(cap.end) || 0) + move).toFixed(2));
    });
    state.captionGlobalOffset = Number(((Number(state.captionGlobalOffset) || 0) + move).toFixed(2));
    state.audioDuration = Math.max(state.audioDuration || 0, ...state.captions.map(c => Number(c.end) || 0));
    log(`전체 자막을 ${move > 0 ? '뒤로' : '앞으로'} ${Math.abs(move).toFixed(1)}초 이동했습니다.`);
    refreshAfterCaptionSyncMove();
  }

  function shiftFirstCaptionStart(delta) {
    const cap = state.captions[0];
    if (!cap) return;
    pushUndoState();
    let move = Number(delta) || 0;
    const oldStart = Number(cap.start) || 0;
    const end = Number(cap.end) || oldStart + 0.18;
    let nextStart = state.captions[1] ? Number(state.captions[1].start) || Infinity : Infinity;
    let next = clamp(oldStart + move, 0, Math.max(0, Math.min(end - 0.18, nextStart - 0.06)));
    move = Number((next - oldStart).toFixed(2));
    if (Math.abs(move) < 0.001) return;
    cap.start = Number(next.toFixed(2));
    state.firstCaptionOffset = Number(((Number(state.firstCaptionOffset) || 0) + move).toFixed(2));
    log(`첫 자막 시작을 ${move > 0 ? '뒤로' : '앞으로'} ${Math.abs(move).toFixed(1)}초 이동했습니다.`);
    refreshAfterCaptionSyncMove(cap);
  }

  function setAllCaptionOffset(target) {
    const current = Number(state.captionGlobalOffset || 0);
    shiftAllCaptions((Number(target) || 0) - current);
  }

  function setFirstCaptionOffset(target) {
    const current = Number(state.firstCaptionOffset || 0);
    shiftFirstCaptionStart((Number(target) || 0) - current);
  }

  function resetCaptionOffsets() {
    if (!state.captions.length) return;
    pushUndoState();
    const allMove = -(Number(state.captionGlobalOffset || 0));
    if (Math.abs(allMove) > 0.001) {
      const minStart = Math.min(...state.captions.map(c => Number(c.start) || 0));
      const safeMove = minStart + allMove < 0 ? -minStart : allMove;
      state.captions.forEach((cap) => {
        cap.start = Number(Math.max(0, (Number(cap.start) || 0) + safeMove).toFixed(2));
        cap.end = Number(Math.max(cap.start + 0.18, (Number(cap.end) || 0) + safeMove).toFixed(2));
      });
    }
    const cap = state.captions[0];
    const firstMove = -(Number(state.firstCaptionOffset || 0));
    if (cap && Math.abs(firstMove) > 0.001) {
      const oldStart = Number(cap.start) || 0;
      const end = Number(cap.end) || oldStart + 0.18;
      const nextStart = state.captions[1] ? Number(state.captions[1].start) || Infinity : Infinity;
      cap.start = Number(clamp(oldStart + firstMove, 0, Math.max(0, Math.min(end - 0.18, nextStart - 0.06))).toFixed(2));
    }
    state.captionGlobalOffset = 0;
    state.firstCaptionOffset = 0;
    log('자막 싱크 보정값을 초기화했습니다.');
    refreshAfterCaptionSyncMove(state.captions[0]);
  }

  ensureCaptionSyncOffsetPanel();
  $('btnSyncAllMinus')?.addEventListener('click', () => shiftAllCaptions(-0.1));
  $('btnSyncAllPlus')?.addEventListener('click', () => shiftAllCaptions(0.1));
  $('btnFirstSyncMinus')?.addEventListener('click', () => shiftFirstCaptionStart(-0.1));
  $('btnFirstSyncPlus')?.addEventListener('click', () => shiftFirstCaptionStart(0.1));
  $('btnSyncReset')?.addEventListener('click', resetCaptionOffsets);
  $('syncOffset')?.addEventListener('change', () => setAllCaptionOffset(parseFloat($('syncOffset').value || '0')));
  $('firstSyncOffset')?.addEventListener('change', () => setFirstCaptionOffset(parseFloat($('firstSyncOffset').value || '0')));
  refreshCaptionSyncInputs();
  ...
  $('btnAutoPhoto')?.addEventListener('click', async () => {
    if (!senialConsumeDailyUse('대본 생성')) return;
    pushUndoState();
    if ($('autoPhotoMenu')) $('autoPhotoMenu').style.display = 'none';
    if ($('autoComicMenu')) $('autoComicMenu').style.display = 'none';
    syncAutoScriptLengthToMain();
