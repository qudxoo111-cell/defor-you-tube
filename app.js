...
function buildCaptionTimeline(parts, totalDuration, style) {
  const total = Math.max(0.5, Number(totalDuration) || 0);
  const minDur = clamp(parseFloat($('captionMinSeconds')?.value || '1.5'), 1.0, 4);
  const durations = distributeCaptionDurations(parts, total, minDur);
  const captions = [];
  let t = 0;
  parts.forEach((text, idx) => {
    ...
    t = end;
    captions.push({ id: uid(), start, end, text, captionStyle: { ...(style || collectCaptionStyle()) } });
  });
  if (captions.length) {
    captions[0].start = 0;
...
function createCaptionsFromScript() {
  ...
  state.captions = buildCaptionTimeline(parts, total, style);
  ...
  log(`V43 자동분할 싱크 엔진 적용 완료: 음성 ${formatTime(total)}, 자막 ${parts.length}개`);
}
...
function buildLocalSeoScript() {
  ...
  const locked = cleanSeoTopicText(state.autoRunLock?.topic || '');
  const picked = cleanSeoTopicText($('scriptTopic')?.value || '');
  const topic = locked || picked || cleanSeoTopicText($('autoSeedTopic')?.value || '') || cleanSeoTopicText(getTopicSuggestions(category)[0] || category);
  const request = String($('scriptRequest')?.value || '').trim();
...
function retimeExistingCaptionsToDuration(totalDuration) {
  ...
  const parts = state.captions
    .flatMap((cap) => splitBlockByLength(removeTailRepeat(cap.text), getChunkLimit()))
    .filter(Boolean);
  if (!parts.length) return false;
  const oldStyles = state.captions.map((cap) => ({ ...(cap.captionStyle || styleFallback) }));
  const oldIds = state.captions.map((cap) => cap.id);
  const rebuilt = buildCaptionTimeline(parts, total, styleFallback);
  ...
  log(`음성 길이 ${formatTime(total)} 기준으로 문장 가중치 싱크를 다시 계산했습니다.`);
  return true;
}
...
function buildSeoScriptPrompt() {
  const category = $('scriptCategory')?.value || '상식';
  const topic = cleanSeoTopicText(state.autoRunLock?.topic || $('scriptTopic')?.value || '').trim();
  const request = String($('scriptRequest')?.value || '').trim();
...
async function runAutoModeFromTopic(version, kind) {
  const runSeq = (state.autoRunSeq || 0) + 1;
  state.autoRunSeq = runSeq;

  const lockedTopic = cleanSeoTopicText(
    $('scriptTopic')?.value || $('autoSeedTopic')?.value || ''
  );

  if (!lockedTopic) {
    log('먼저 올자동 주제를 고르세요. 선택한 주제만 실행됩니다.');
    return;
  }

  state.autoRunLock = { version, kind, topic: lockedTopic, runSeq };
  if ($('scriptTopic')) $('scriptTopic').value = lockedTopic;
  if ($('autoSeedTopic')) $('autoSeedTopic').value = lockedTopic;

  setImageStyleFromAuto(version, kind);
  log(`올자동 ${version === 'photo' ? '실사' : '만화'} 실행: ${lockedTopic}`);

  const script = await generateSeoScriptByApi();
  if (state.autoRunSeq !== runSeq) return;

  const text = $('rightScriptEditor')?.value || $('generatedScript')?.value || script || '';
  if (text) {
    setScriptIntoEditors(text);
    createCaptionsFromScript();
  }
  ...
  if (state.autoRunSeq !== runSeq) return;
  // 이미지 생성 과정에서 프롬프트가 클립보드를 덮어쓸 수 있으므로 마지막에 대본을 다시 복사한다.
  if (text) await navigator.clipboard?.writeText(text).catch(() => {});
  const panel = $('autoModePanel');
  if (panel) panel.style.display = 'none';
  state.autoRunLock = null;
  log('올자동 셋팅 완료. 선택한 주제와 선택한 스타일만 실행했습니다.');
}
...
function bindEvents() {
  ...
  $('btnApplyRightScript')?.addEventListener('click', () => { pushUndoState(); syncScriptEditors('right'); createCaptionsFromScript(); });
  ...
  $('btnTranscribe')?.addEventListener('click', () => {
    ...
    if (audio && audio.duration) {
      if (!state.captions.length && ($('rightScriptEditor')?.value || '').trim()) {
        syncScriptEditors('right');
        createCaptionsFromScript();
      }
      const changed = retimeExistingCaptionsToDuration(audio.duration);
      if (!changed) {
        state.audioDuration = audio.duration;
        updateSeekUI(0);
        drawFrame(0);
      }
      log(changed ? '음성 길이에 맞춰 문장 가중치 싱크를 다시 계산했습니다.' : '음성 길이를 영상 길이에 적용했습니다.');
    } else {
      log('먼저 음성 파일을 첨부하세요.');
    }
  });
  ...
  function applyAudioFile(file) {
    ...
    audio.onloadedmetadata = () => {
      ...
      const changed = retimeExistingCaptionsToDuration(state.audioDuration);
      if (!changed) {
        updateSeekUI(0);
        drawFrame(0);
      }
      log(changed ? '음성 파일이 들어갔고, 문장 가중치 싱크를 다시 계산했습니다.' : '음성 파일이 들어갔습니다. 재생 버튼으로 확인하세요.');
    };
  }
  ...
  document.querySelectorAll('[data-auto-version][data-auto-kind]').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      ...
      btn.disabled = true;
      try {
        await runAutoModeFromTopic(btn.dataset.autoVersion, btn.dataset.autoKind);
      } finally {
        btn.disabled = false;
      }
    });
